            ...
            <div className="flex justify-between items-start mb-6">
              ...
              <div className="w-12 h-12 rounded-xl flex items-center justify-center bg-[#2d2d2d] border border-[#3d3d3d]">
                {company.logoType === 'text' ? (
                  <span className="text-white font-bold text-sm bg-white text-black w-full h-full flex items-center justify-center rounded-xl">
                    {company.logoText}
                  </span>
                ) : (
                  company.logoIcon
              ...
              {/* স্ট্যাটাস ব্যাজ */}
              <span
                className={`flex items-center gap-1 text-[11px] font-bold tracking-wider px-2.5 py-1 rounded-md border ${
                  company.status === 'VERIFIED'
                    ? 'bg-[#142918] text-[#4ade80] border-[#1e4620]'
                    : 'bg-[#2a2115] text-[#fbbf24] border-[#453319]'
                }`}
              >
                {company.status === 'VERIFIED' ? (
                  <FiCheckCircle className="text-xs" />
                ) : (
                  <MdOutlinePendingActions className="text-xs" />
                )}
                {company.status}
              </span>
            </div>
