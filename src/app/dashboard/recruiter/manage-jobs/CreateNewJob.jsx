...
export default function CreateNewJobModal({ recruiterCompany, user }) {
  ...
  const maxLimit =
    companyInfo.plan === "Free" ? 3 : companyInfo.plan === "Growth" ? 10 : 50;
  const isPostingAllowed =
    companyInfo.isApproved && companyInfo.activeJobsCount < maxLimit;
  ...
  const handleSubmit = async (e) => {
    ...
    if (!formProps.jobTitle?.trim())
      validationErrors.jobTitle = "Job title is required";
    if (!selectedCategory)
      validationErrors.jobCategory = "Please select a job category";
    if (!selectedJobType) validationErrors.jobType = "Please select a job type";
    if (!formProps.minSalary)
      validationErrors.minSalary = "Minimum salary is required";
    if (!formProps.maxSalary)
      validationErrors.maxSalary = "Maximum salary is required";
    if (!isRemote && !formProps.jobLocation?.trim())
      validationErrors.jobLocation =
        "Job location is required for non-remote roles";
    if (!formProps.applicationDeadline)
      validationErrors.applicationDeadline = "Application deadline is required";
    if (!formProps.responsibilities?.trim())
      validationErrors.responsibilities = "Job responsibilities are required";
    if (!formProps.requirements?.trim())
    ...
    const isapprovedcompany = recruiterCompany.filter((item)=> item._id === payload.companyId);
    console.log(isapprovedcompany, "this is my")
    if(isapprovedcompany.hasOwnProperty()){
      ...
      toast.danger("you company is not approved")
    }
  };
          ...
          >
            ...
            <div className="flex items-center justify-between px-6 pt-6 pb-4 border-b border-white/5">
              <div className="text-left">
                ...
                <p className="text-xs text-gray-500 font-light mt-0.5">
                  Enter your business and job details to start hiring on
                  HireLoop.
                </p>
              </div>
              <Button
            ...
            <div className="px-6 py-5 grid grid-cols-1 sm:grid-cols-2 gap-4 max-h-[70vh] overflow-y-auto overflow-x-visible">
              ...
              <TextField className="sm:col-span-2 w-full flex flex-col gap-1.5 text-left">
                ...
                />
                {errors.jobTitle && (
                  <span className="text-[11px] text-red-500 font-light mt-0.5">
                    {errors.jobTitle}
                  </span>
                )}
              </TextField>
              ...
              >
                ...
                >
                  <span
                    className={
                      SelectedCompany ? "text-gray-200" : "text-gray-500"
                    }
                  >
                    {SelectedCompany || "Select Company"}
                  </span>
                  <FiChevronDown
                    className={`text-gray-500 text-sm transition-transform duration-150 ${IsCompanyOpen ? "rotate-180" : ""}`}
                  />
                </button>
                {IsCompanyOpen && (
                  <div className="absolute top-[62px] left-0 w-full bg-[#212127] border border-white/50 rounded-lg shadow-xl py-1 z-50 max-h-40 overflow-y-auto">
                    {recruiterCompany.map((item) =>
                      ...
                      //   { item?.status === "approved" ? item?.companyName: ""}
                      // </button>
                    )}
                  </div>
                )}
              ...
              >
                ...
                >
                  <span
                    className={
                      selectedCategory ? "text-gray-200" : "text-gray-500"
                    }
                  >
                    {selectedCategory || "Select Category"}
                  </span>
                  <FiChevronDown
                    className={`text-gray-500 text-sm transition-transform duration-150 ${isCategoryOpen ? "rotate-180" : ""}`}
                  />
                </button>
                {isCategoryOpen && (
                  <div className="absolute top-[62px] left-0 w-full bg-[#212127] border border-white/50 rounded-lg shadow-xl py-1 z-50 max-h-40 overflow-y-auto">
              ...
              >
                ...
                >
                  <span
                    className={
                      selectedJobType ? "text-gray-200" : "text-gray-500"
                    }
                  >
                    {selectedJobType || "Select Job Type"}
                  </span>
                  <FiChevronDown
                    className={`text-gray-500 text-sm transition-transform duration-150 ${isJobTypeOpen ? "rotate-180" : ""}`}
                  />
                </button>
                {isJobTypeOpen && (
                  <div className="absolute top-[62px] left-0 w-full bg-[#212127] border border-white/50 rounded-lg shadow-xl py-1 z-50 max-h-40 overflow-y-auto">
              ...
              <div className="sm:col-span-2 grid grid-cols-3 gap-3 items-start">
                <TextField className="w-full flex flex-col gap-1.5 text-left">
                  ...
                  <div
                    className={`flex items-center bg-[#212127] hover:bg-[#26262d] focus-within:bg-[#26262d] h-10 px-3 rounded-lg border ${errors.minSalary ? "border-red-500/50 focus-within:border-red-500" : "border-transparent focus-within:border-white/50"} transition-colors w-full`}
                  >
                    ...
                    <Input
                      ...
                      placeholder="Min"
                      className="text-xs text-gray-200 bg-transparent outline-none w-full"
                    />
                  </div>
                  {errors.minSalary && (
                    <span className="text-[11px] text-red-500 font-light mt-0.5">
                ...
                <TextField className="w-full flex flex-col gap-1.5 text-left">
                  ...
                  <div
                    className={`flex items-center bg-[#212127] hover:bg-[#26262d] focus-within:bg-[#26262d] h-10 px-3 rounded-lg border ${errors.maxSalary ? "border-red-500/50 focus-within:border-red-500" : "border-transparent focus-within:border-white/50"} transition-colors w-full`}
                  >
                    <FiDollarSign className="text-gray-500 text-xs shrink-0 mr-1" />
                    <Input
                      type="number"
              ...
              <div className="sm:col-span-2 grid grid-cols-1 sm:grid-cols-3 gap-4 items-start">
                <TextField
                  className={`sm:col-span-2 w-full flex flex-col gap-1.5 text-left transition-opacity ${isRemote ? "opacity-30 pointer-events-none" : "opacity-100"}`}
                >
                  <Label className="text-xs font-medium text-gray-400">
                    Job Location {!isRemote && "*"}
                  </Label>
                  <div
                    className={`flex items-center bg-[#212127] h-10 px-3 rounded-lg border ${!isRemote && errors.jobLocation ? "border-red-500/50" : "border-transparent focus-within:border-white/50"} transition-colors w-full`}
                  >
              ...
              <TextField className="w-full flex flex-col gap-1.5 text-left">
                ...
                <div
                  className={`flex items-center bg-[#212127] h-10 px-3 rounded-lg border ${errors.applicationDeadline ? "border-red-500/50" : "border-transparent"} w-full`}
                >
                  ...
                  <Input
                    ...
                    className="text-xs text-gray-200 bg-transparent outline-none w-full [color-scheme:dark]"
                  />
                </div>
              ...
              {!isPostingAllowed && (
                <div className="sm:col-span-2 text-left bg-red-500/10 border border-red-500/20 text-red-400 p-3 rounded-lg text-xs font-light">
                  ⚠️ Cannot post job. Limit exceeded (
                  {companyInfo.activeJobsCount}/{maxLimit} for{" "}
            ...
            <div className="px-6 py-4 border-t border-white/5 flex items-center justify-end gap-3 bg-[#1e1e24]/40">
              <Button
                type="button"
                onClick={handleModalClose}
                className="bg-[#212127] text-gray-300 text-xs px-5 h-9 rounded-lg"
              >
                Cancel
              </Button>
              <Button
                ...
                className={`text-xs font-semibold px-5 h-9 rounded-lg ${isPostingAllowed ? "bg-white text-[#0d0d11]" : "bg-white/10 text-gray-500 cursor-not-allowed"}`}
              >
                Register & Post Job
              </Button>
            </div>
